...
def T_gcy2(w, shapes, params, arrays):
    ...
    A1= np.expand_dims(A1, list(ms-fs))
    ...
    A2= np.expand_dims(A2, list(ms-fs))
    ...
    indices_a[state_numbers['h_zπ']] = n_h_zπ
    indices_a[state_numbers['z']] = n_z
    fs = {state_numbers['z_π'], state_numbers['h_z'],state_numbers['h_zπ'],
          state_numbers['z'] }
    indices = indices_a + indices_b
    A3 = np.swapaxes(A3, 2, 3)
    A3 = np.swapaxes(A3, 1, 2)
    A3 = np.swapaxes(A3, 0, 1)
    A3= np.expand_dims(A3, list(ms-fs))
    ...
    h_z_Q= np.expand_dims(h_z_Q, list(ms-fs))
    ...
    h_zπ_Q= np.expand_dims(h_zπ_Q, list(ms-fs))
    ...
    h_λ_Q= np.expand_dims(h_λ_Q, list(ms-fs))
    ...
    h_c_Q= np.expand_dims(h_c_Q, list(ms-fs))
    ...
    indices_b = [1] * n
    indices_b[state_numbers['z_π']] = n_z_π
    fs = {state_numbers['h_zπ'], state_numbers['z_π'], state_numbers['jz_π']}

    #z_π_Q = np.reshape(z_π_Q, indices_a + indices_b)
    z_π_Q = np.swapaxes(z_π_Q, 0, 1)
    z_π_Q= np.expand_dims(z_π_Q, list(ms-fs))
    ...
    z_Q= np.expand_dims(z_Q, list(ms-fs))
...
def T_gcy_loops(w, shapes, params, arrays):
    ...
    (z_states,      z_Q,
     ...
     h_zπ_states, h_zπ_Q, σ_zπ_states,
     h_λ_states,  h_λ_Q)   = arrays
    # print("line -442", z_π_Q.shape, n_z)
    θ = (1 - γ) / (1 - 1/ψ)
    Hwθ = np.empty(shapes)
    foo=[]
...
# fails: 2, 3, 3, 2, 3, 2
def test_vectorized_equals_loops(shapes=(3, 4, 5, 2, 6, 7)):
    """
